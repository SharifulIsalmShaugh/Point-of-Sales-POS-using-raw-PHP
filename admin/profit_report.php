	...
	<section>
		...
		<thead>
			...
			if(isset($_POST['button'])){
				$from = $_POST['d1'];
				$to = $_POST['d2'];
				$query="SELECT sales_details.*,products.* FROM sales_details,products WHERE sales_details. 
        ...
        while ($result = mysqli_fetch_assoc($exquery)) {
           $i++;
                ...
                <td><?php  
               $stotal = ($result['sale_price'] - $result['buy_price'])*$result['product_quantity'];
               echo $stotal;?>
                TK</td>
				...
				<tfoot>
				<tr>
					<td style="text-align: center; font-weight: bold;" colspan="5">Total Profit</td>
					<td style="font-weight: bold;" colspan="3"><?php echo $sum;?> Tk</td>
				</tr></tfoot>
			</table>
